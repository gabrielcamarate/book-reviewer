...
class ExportDocxTest(unittest.TestCase):
    def test_export_manuscript_docx_writes_ptbr_output_from_current_state(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("deliverable_manifest_path", summary)
            snapshot_manifest_path = Path(summary["snapshot_manifest_path"])
            deliverable_manifest_path = Path(summary["deliverable_manifest_path"])
            self.assertTrue(snapshot_manifest_path.exists())
            self.assertTrue(deliverable_manifest_path.exists())
            ...
            self.assertEqual(deliverable_manifest["language"], "pt-BR")
            self.assertEqual(deliverable_manifest["output_path"], str(output_path))
            self.assertIn("sha256", deliverable_manifest)
            self.assertEqual(deliverable_manifest["snapshot_manifest_path"], str(snapshot_manifest_path))
    ...
    def test_export_manuscript_docx_writes_spanish_output_when_translations_cover_all_paragraphs(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            deliverable_manifest = json.loads(
                Path(summary["deliverable_manifest_path"]).read_text(encoding="utf-8")
            )
            self.assertEqual(deliverable_manifest["language"], "es")
            self.assertEqual(deliverable_manifest["deliverable_slug"], "es")
            self.assertIn("sha256", deliverable_manifest)
